 ...
 * Zodiac Logic Engine
 *
 * Sun Sign derivation and compatibility mapping (Friends/Enemies).
 */
...
export function getSunSign(dob: string): ZodiacSignName {
...
const ZODIAC_COMPATIBILITY: Record<
  ZodiacSignName,
  {
    friends: { sign: ZodiacSignName; description: string }[];
    enemies: { sign: ZodiacSignName; description: string }[];
  }
> = {
  Aries: {
    friends: [
  ...
  Scorpio: {
    ...
    enemies: [
      { sign: "Leo", description: "Ego and power clashes are frequent." },
      {
        sign: "Aquarius",
...
export function getZodiacRelations(sign: ZodiacSignName) {
